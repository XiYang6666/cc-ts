 ...
 *
 * DFPWM (Dynamic Filter Pulse Width Modulation) is an audio codec designed by
 * GreaseMonkey. It's a relatively compact format compared to raw PCM data, only
 * using 1 bit per sample, but is simple enough to encode and decode in real
 * time.
 *
 * Typically DFPWM audio is read from the filesystem or a a web request as a
 * string, and converted a format suitable for `speaker.playAudio`.
 *
 * ### Encoding and decoding files
 * This module exposes two key functions, {@link make_decoder} and
 * {@link make_encoder}, which construct a new decoder or encoder. The returned
 * encoder/decoder is itself a function, which converts between the two kinds of
 * data.
 *
 * These encoders and decoders have lots of hidden state, so you should be
 * careful to use the same encoder or decoder for a specific audio stream.
 * Typically you will want to create a decoder for each stream of audio you
 * read, and an encoder for each one you write.
 *
 * ### Converting audio to DFPWM
 * DFPWM is not a popular file format and so standard audio processing tools may
 * not have an option to export to it. Instead, you can convert audio files
 * online using `music.madefor.cc`, the `LionRay Wav Converter` Java application
 * or `FFmpeg 5.1` or later.
 *
 * @example
 ...
 * }
 * ```
 * @see {@link Playing audio with speakers} Gives a more general introduction to
 * audio processing and the speaker.
...
declare module "cc.audio.dfpwm" {
   ...
   * amplitude data between -128 and 127 and returns the encoded DFPWM data.
   *
   * > [!WARNING] Reusing encoders Encoders have lots of internal state which
   * tracks the state of the current stream. If you reuse an encoder for
   * multiple streams, or use different encoders for the same stream, the
   * resulting audio may not sound correct.
   *
   * @returns The encoder function: `(pcm: number[]) => string`.
   * @see {@link encode} A helper function for encoding an entire file of audio
   * at once.
   */
   ...
   *
   * The returned decoder is itself a function. This function accepts a string
   * and returns a table of amplitudes, each value between -128 and 127.
   *
   * > [!WARNING] Reusing decoders Decoders have lots of internal state which
   * tracks the state of the current stream. If you reuse an decoder for
   * multiple streams, or use different decoders for the same stream, the
   * resulting audio may not sound correct.
   *
   ...
   * }
   * ```
   * @see {@link decode} A helper function for decoding an entire file of audio
   * at once.
   */
   ...
   * the file in chunks and process it using {@link make_decoder}.
   *
